...
const ProfileImage: React.FC<ProfileImageProps> = ({ src, alt, className = '' }) => {
  return (
    <div className={`relative group ${className}`}>
      ...
      <div className="absolute inset-0 bg-card rounded-full opacity-0 group-hover:opacity-10 transition-opacity duration-300"></div>
      <img 
        src={src} 
        alt={alt} 
        className="w-full h-full object-cover rounded-full shadow-lg transition-transform duration-300 group-hover:scale-105"
      />
      <div className="absolute inset-0 rounded-full border-2 border-foreground/50 opacity-0 group-hover:opacity-100 animate-pulse"></div>
    </div>
  );
};
